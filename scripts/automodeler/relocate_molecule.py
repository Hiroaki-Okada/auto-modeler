...
class RelocateMolecule(ReadInput):
    ...
    def run(self, c_name_comb, ref_atom_xyz):
        ...
        for X_idx in self.mol_mode_X_inx:
            xyz_opt_atom_xyz = self.xyz_axis_opt(X_idx, ref_atom_xyz, c_name_comb)
            xyz_opt_atom_xyz = [i[:1] + [float(j) for j in i[1:]] for i in xyz_opt_atom_xyz]
            best_atom_xyz[X_idx] = xyz_opt_atom_xyz
    ...
    def translation(self, atom_xyz, X_xyz):
        ...
        for i in range(len(atom_xyz)):
            for idx in range(1, 4):
                atom_xyz[i][idx] += moving_dist[idx - 1]

        atom_xyz = tuple([tuple(i) for i in atom_xyz])

        return atom_xyz

    def xyz_axis_opt(self, X_idx, ref_atom_xyz, c_name_comb):
        ...
        for mol_X_idx in self.mol_mode_X_inx:
            atom_xyz = self.model_X_atom_xyz[mol_X_idx]
